...
```swift
let url = URL(string: "...")
imageView.sd_setImage(with: url, completed: nil) // UIImageView+WebCache
```

`@interface UIImageView (WebCache)` 中完整参数的接口（其余接口只有部分参数）：
...
`SDWebImageOptions` 定义在 `SDWebImageDefine.h`，是图片下载的选项

```objc
typedef NS_OPTIONS(NSUInteger, SDWebImageOptions) {
    /**
     * By default, when a URL fail to be downloaded, the URL is blacklisted so the library won't keep trying.
     * This flag disable this blacklisting. （允许失败重试）
     */
    SDWebImageRetryFailed = 1 << 0,
}
```

`SDWebImageContext` 定义在 `SDWebImageDefine.h`，是对下载选项的一个扩充，This hold the extra objects which options enum can not hold. 它是一个字典：

```objc
typedef NSDictionary<SDWebImageContextOption, id> SDWebImageContext;
```

通过这个字典，可以对图片 operation key、cache、loader、coder、transformer 等进行自定义。

再看看下载进度和下载完成回调 block 的定义：
...
- (void)sd_setImageWithURL:(nullable NSURL *)url
    ...
    // SDInternalCompletionBlock 多出来的两个参数没有使用到。
    // 应该是别的地方有用
...
NSMapTable 可以对键和值的弱引用，此时当键或者值当中的一个被释放时，这一键值对就会被移除掉
...
自定义项的优先顺序处理，类似于 [Git Configuration](https://git-scm.com/book/en/v2/Customizing-Git-Git-Configuration) 的分级处理，高优先级的选项会覆盖低优先级的选项。
...
- `- (void)callCacheProcessForOperation:`
  - 缓存图片查找结果返回，进入下载流程
    - 没有获取到缓存图片，有 context 指定的 transformer，进入一个特别的流程处理
...
SDImageCacheDefine.h/m 定义了协议 `@protocol SDImageCache`，外部可以遵守这个协议并自行实现协议中的方法。如果不自定义的话，则会使用 SDImageCache.h/m 中框架默认的实现类。
...
- 线程安全，无需加锁。You can add, remove, and query items in the cache from different threads without having to lock the cache yourself.
- 对 key 强引用，而不是 copy
...
注意在 SDImageCacheConfig 中有一个选项 shouldUseWeakMemoryCache 默认开启。它的说明：

`SDImageCache`'s memory cache will use a weak maptable to store the image at the same time when it stored to memory, and get removed at the same time. 当图片存入内存缓存的同时，也在这个哈希表里保存了一份弱引用，当内存缓存被清除时会一并清除。

However when memory warning is triggered, since the weak maptable does not hold a strong reference to image instance, even when the memory cache itself is purged, some images which are held strongly by UIImageViews or other live instances can be recovered again, to avoid later re-query from disk cache or network. This may be helpful for the case, for example, when app enter background and memory is purged, cause cell flashing after re-enter foreground. 但是，当内存缓存被清除时，有一些图片仍然会被正在显示的控件如 UIImageView 强引用，内存不会回收，这时 weak memory cache 就起作用了。可以在下次查找缓存时命中，而无需读取磁盘。
...
- 擦出内存缓存：调用 NSCache 和 NSMapTable 的 `removeObjectForKey` 方法。
...
文件的增删改查，使用了 NSFileManager 和 NSData 的常规方法，这里就不展开说了。

SDDiskCache 实现了计算文件占用大小、文件个数的相关方法。
...
+ (NSData *)extendedAttribute:(NSString *)name atPath:(NSString *)path traverseLink:(BOOL)follow error:(NSError **)err {
    ...
    // 符号链接（Symbolic Links）：是一个指向其他地方某个文件的指针，即快捷方式！
...
func downsample(imageAt imageURL: URL, to pointSize: CGSize, scale: CGFloat) -> UIImage {
    /*
     * kCGImageSourceShouldCache Specifies whether the image should be cached in a decoded form. The
...
```swift
// Avoiding thread explosion 线程爆炸 when doing asynchronous work
...
By default, we will decode the image in the background during cache query and download from the network. This can help to improve performance because when rendering image on the screen, it need to be firstly decoded. But this happen on the main queue by Core Animation. However, this process may increase the memory usage as well. 默认情况下，将图片渲染在屏幕上，需要先由 Core Animation 在主线程进行解码。SDWebImage 在查询缓存和下载图片时帮我们在后台线程进行解码，这可以提高性能，但会增加内存占用。
