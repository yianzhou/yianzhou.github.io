...
一个视频文件包含图像和音频，还有字幕、配置信息等，这些内容需要按照一定的规则组织起来，即封装。常见的封装格式有 MP4、FLV、AVI、MOV、MKV、3GP 等等。
...
B 帧：双向预测帧。参考前后的帧来生成一个完整的画面。直播流中通常会放弃 B 帧，原因就是 B 帧需要向后预测，会延长直播的时延。
...
不同的码流（360P、720P、1080P 等）如何无缝切换：主流的方案是两个播放器，切换时，旧播放器保持播放，新播放器拉流并解码，缓冲并解码完成后，在旧播放器即将播放下一个 I 帧时，切换的新播放器对应位置的 I 帧上，实现无缝连接。
