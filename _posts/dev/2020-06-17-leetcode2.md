...
rand(7) - 1 是 [0, 6] 之间随机，(rand(7) - 1) \* 7 是 [0, 7, 14, 21, 28, 35, 42] 之间随机。(rand(7) - 1) \* 7 + rand(7) 就得到 [1, 49] 之间的随机数。

存在这样一个规律，`(rand(X) - 1) * Y + rand(Y) = rand(X*Y)`，利用这个规律，我们可以得到 rand(49)。

对于 > 40 的数字我们直接丢弃（拒绝采样），剩余 [1, 40] 等概率出现。rand(40) % 10 + 1 即可以得到 rand(10)。得到第一个 Accepted 的版本：
...
上一轮拒绝采样的是 [1, 9] 内的随机数，利用上面的公式可以得到 rand(7\*9) = rand(63)。保留 [1, 60]，拒绝 [1, 3]，再次得到 rand(21)。此时保留 [1, 20]，舍弃 21 即可。虽然理论上的最坏时间复杂度还是 O(∞)，但实际上我们降低了调用 rand7 函数的期望次数。
...
`void put(int key, int value)` 如果 key 存在则更新其 value；否则插入 key-value。当超出容量时，移除最久未使用的值。

实现本题的两种操作，需要用到一个哈希表和一个双向链表。Java 里面的 `LinkedHashMap` 就可以处理这样的问题。本题需要我们自己实现这样的结构。

哈希表通过键映射到双向链表中的位置。

双向链表按照被使用的顺序存储键值对，靠近头部的键值对是最近使用的，靠近尾部的键值对是最久未使用的。
...
private:
    int capacity;                            // max capacity of this container
    int size;                                // current size of this container
    unordered_map<int, shared_ptr<Node>> um; // key to node in doubly linked list mapping
    shared_ptr<Node> head;                   // head in doubly linked list
    shared_ptr<Node> tail;                   // tail in doubly linked list
    void addToHead(shared_ptr<Node> node);   // 将节点插入到双向链表的头部
    void moveToHead(shared_ptr<Node> node);  // 将节点移动到双向链表的头部
    void removeNode(shared_ptr<Node> node);  // 辅助函数，删除双向链表的一个节点
    shared_ptr<Node> removeTail();           // 移除双向链表的尾巴
};

LRUCache::LRUCache(int c) : capacity(c), size(0), head(make_shared<Node>()), tail(make_shared<Node>())
{
    // 伪头部和尾部节点，不实际存储键值对
    head->next = tail;
    tail->next = head;
...
void LRUCache::addToHead(shared_ptr<Node> node)
{
    node->prev = head;
    node->next = head->next;
    head->next->prev = node;
...
void LRUCache::moveToHead(shared_ptr<Node> node)
{
    removeNode(node);
    addToHead(node);
}

void LRUCache::removeNode(shared_ptr<Node> node)
{
    node->next->prev = node->prev;
    node->prev->next = node->next;
}

shared_ptr<Node> LRUCache::removeTail()
{
    shared_ptr<Node> node = tail->prev; // 注意，这才是真正的尾巴！
    removeNode(node);
    return node;
}
```
