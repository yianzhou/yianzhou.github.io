...
在零钱找换问题中，我们只需要考虑一个维度，即金额，如何用给定的硬币，组合成需要的金额，这类问题我们使用一个 dp 数组，自底向上构建出问题的答案。接下来我们看一类需要考虑两个维度的问题，即背包问题，我们的 dp table 也扩展到了二维数组 dp[i][j]。

## 0-1 背包问题

背包的容量为 W，给定一组物品，第 i 个物品的重量为 weight[i]、价值为 val[i]，问这个背包最多能装的价值是多少？

这是一个典型的动态规划问题。对于一个物品，它只有两种选择，要么放进背包，要么不放，所以也称 0-1 背包问题。

一、找最优子结构——问什么就设什么。题目问的是 N 个物品装进容量为 W 的背包，我们就设 dp[i][j] 为前 i 个物品装进容量为 j 的背包可以装的最大价值，根据这个定义，我们想求的最终答案就是 dp[N][w]。

二、找状态转移方程——考虑第 i 个物品如何从之前的状态转移过来？分为两种情况：

- 第 i 个物品没有放进背包：dp[i][j] = dp[i-1][j]，直接继承之前的结果。
- 第 i 个物品放进了背包：dp[i][j] = dp[i-1]j-[weight[i-1]]] + val[i-1]，即剩余重量 j - weight[i-1] 限制下的最大价值，加上第 i 个物品的价值 val[i-1]。
