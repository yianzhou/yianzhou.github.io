...
内存是一个巨大的字节数组，从理论上讲，对变量的访问可以从任何地址开始，但是实际上，计算机并不是按字节来存取内存的，而是以双字节、四字节、8 字节、16 字节这样来存取内存，称为内存存取粒度。因此，编译器会对基本数据类型的合法地址作出一些限制，即内存地址必须是 2、4、8 的倍数，称为内存对齐。iOS 在 64 位架构下，对齐系数是 8 个字节。
...
![img](/assets/images/173DD679-9075-45AE-B51F-EC4B634F468D.jpg)

![img-60](/assets/images/43C227F1-EA8B-42FC-872D-88E3E68EE0EC.jpg)
...
注意，这个是方法交换的常规写法，但它在某些情况下可能达不到我们预期的效果，更好的做法是引入 [Aspects](https://github.com/steipete/Aspects)、[RSSwizzle](https://github.com/rabovik/RSSwizzle) 之类的库。
