...
block 中修改捕获的自动变量（如上面的`int a`）有两种方法：

1\. 使用静态变量、静态全局变量、全局变量。自动变量是在存储在栈上的，当超出其作用域时，会被栈释放。而静态变量是存储在堆上的，超出作用域时，静态变量没有被释放，所以还可以访问。

2\. 添加 \_\_block 修饰符（存储域说明符）。存储域说明符会指定变量存储的域，如栈 auto、堆 static、全局 extern，寄存器 register。
...
- 手动调用 `copy` 时
- 将 Block 作为函数参数、返回值时
- 将 Block 赋值给 copy 修饰的 id 类或者 Block 类型成员变量，或者 \_\_strong 修饰的自动变量
