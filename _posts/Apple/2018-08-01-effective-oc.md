...
Objective-C 按照 .h 和 .m 的方式组织文件。除非确有必要，否则不要引入头文件。尽量在类的头文件中使用向前声明来提及别的类，尽量降低类之间的耦合。
...
不要用预处理指令：`#define ANIMATION_DURATION 0.3`，请使用类型常量，后者提供了更多的可读性。

若常量是在某个 implementation file 之内，则在名称前加字母 k：
```
