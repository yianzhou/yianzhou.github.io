...
The **pageSize** field in the jetsam event report header records the number of bytes in each memory page.
...
Pages are typically 16KB in size, and they can come in **clean** or **dirty**.
...
Memory-mapped files: if you use read-only files, these are always going to be clean pages.

A typical app, their footprint and profile has a dirty, a compressed, and a clean segment of memory.
