...
自己创建自动释放池可以控制内存峰值不至于过高。比如一个要执行很多次的 for 循环，或者是循环次数未知、需要取决于用户输入的情况，如要从数据库中读取出很多对象，并存放到数组中时。如果把循环内的代码包裹在自动释放池 block 里，那么在循环中自动释放的对象就会被放在这个池子里，而不是在线程的主池子里。例如：
