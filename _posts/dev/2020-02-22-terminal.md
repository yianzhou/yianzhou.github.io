...
终端开启自动补全：`code ~/.inputrc`；Paste the following on separate lines: `set completion-ignore-case on set show-all-if-ambiguous on TAB: menu-complete`
...
显示文件夹目录树：一、安装插件 `brew install tree` 二、可指定层级深度 `tree -L 2`
...
强制切到远端最新：`git checkout -B master origin/master`ƒ
