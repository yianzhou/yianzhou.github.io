...
def isMatch(s, p):
    ...
    firstC = len(s) != 0 and s[0] == p[0]
    return firstC and isMatch(s[1:], p[1:]) # 第一个字符相同，剩下的字符串同理。
```
...
def isMatch(s, p):
    if len(p) == 0: return len(s) == 0
    firstC = len(s) != 0 and p[0] in {s[0], '.'} # . 可以代表任何单个字符
    return firstC and isMatch(s[1:], p[1:])
```
...
class Solution:
    def isMatch(self, s: str, p: str) -> bool:
        if len(p) == 0: return len(s) == 0
        firstC = len(s) != 0 and p[0] in {s[0], '.'}
        if len(p) >= 2 and p[1] == '*':
            # 当前发现 * 通配符
            # p[0] 匹配零次：self.isMatch(s, p[2:])
            # p[0] 匹配第一个字符：firstC and self.isMatch(s[1:], p)
...
class Solution:
    ...
    def match(self, s, p, i, j, cache):
        ...
        if j < len(p) - 1 and p[j+1] == '*':
            cache[(i, j)] = self.match(s, p, i, j+2, cache) or (firstC and self.match(s, p, i+1, j, cache))
            return cache[(i, j)]
        else:
