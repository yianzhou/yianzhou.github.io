...
You notice when `CGContextDrawPath` returns `drawRect:` returns. The way it returns is it pops the stack frame, restores the previous value of the frame Pointer and jumps back to the caller.
