...
int main(int argc, const char * argv[]) {
    @autoreleasepool {
        NSArray<NSNumber *> *nums = @[@-2, @1, @-3, @4, @-1, @2, @1, @-5, @4]; // NSArray 只能存放对象，不能存放基本类型！因此元素为 <NSNumber *>！
        NSMutableArray<NSNumber *> *dp = [NSMutableArray arrayWithCapacity:[nums count]];
        NSLog(@"%lu", (unsigned long)dp.count); // 只初始化容量，不添加元素的话，count == 0
        for (int i = 0; i < nums.count; i++) {
            dp[i] = nums[0];
        }
        NSNumber *res = [NSNumber numberWithLong:0]; // 指针类型！不是基本类型！不能直接 = 0
        for (int i = 1; i < dp.count; i++) {
            if (dp[i-1].longValue >= 0) { // NSNumber.longValue
                dp[i] = [NSNumber numberWithLong:(dp[i-1].longValue + nums[i].longValue)];
            } else {
                dp[i] = [NSNumber numberWithLong:(nums[i].longValue)];
            }
            res = res.longValue > dp[i].longValue ? res : dp[i];
        }
        NSLog(@"%ld", res.longValue);
    }
    return 0;
}
```
