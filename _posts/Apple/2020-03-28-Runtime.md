...
访问数组内元素有两种接口，一种是下标访问 `arr[1]`，实际上是调用了 `objectAtIndexedSubscript:`；一种是 NSArray 的实例方法 `[arr objectAtIndex:1]`

两者均调用到底层类簇中 `-[__NSArray0 objectAtIndex:]` 方法。

因此，可以为 NSArray 创建一个分类：`NSArray+Safe.h/m`，添加安全的索引方法。

```objc
+ (void)load {
    // 注意！NSArray 背后是一个类簇
    Method m1 = class_getInstanceMethod(objc_getClass("__NSArrayI"), @selector(objectAtIndex:));
    Method m2 = class_getInstanceMethod(self, @selector(safeObjectAtIndex:));
