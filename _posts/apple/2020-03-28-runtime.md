...
内存是一个巨大的字节数组，从理论上讲，对变量的访问可以从任何地址开始，但是实际上计算机系统要求数据存放的内存首地址是某个数（通常为 4 或 8）的倍数，这就是所谓的内存对齐。其原因是大部分处理器并不是按字节块来存取内存的，而是以双字节、四字节、8 字节、16 字节甚至 32 字节为单位来存取内存，称为内存存取粒度。
