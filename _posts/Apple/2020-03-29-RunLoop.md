...
1. RunLoop 是一个事件处理循环，当有工作处理时唤醒线程，没有工作时让线程休眠，释放计算资源。
2. 每个线程都有关联的 RunLoop 对象，主线程的 RunLoop 默认开启，子线程的需要手动开启。
3. RunLoop 事件源有两种。
4. 主线程 RunLoop 有两个 Mode。
5. RunLoop 准备进入休眠时，释放旧的池并创建新池。
