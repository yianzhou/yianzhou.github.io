...
- (void)addMetadataItem:(AVMetadataItem *)item withKey:(id)key {
    ...
    if (normalizedKey) {
        id <THMetadataConverter> converter =
            [self.converterFactory converterForKey:normalizedKey]; // 1
...
时间相关的元数据只用于资源的时间轴内特定的一个范围。`AVTimedMetadataGroup`: A collection of metadata items that are valid for use during a specific time range. For example, AVTimedMetadataGroups are used to represent chapters, optionally containing metadata items for chapter titles and chapter images.
