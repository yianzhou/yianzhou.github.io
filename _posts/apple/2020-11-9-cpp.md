...
编译并链接：`g++ factMain.cpp fact.cpp`，产出 `a.out` 文件。

等价于下面的步骤（分离式编译）：

1. 只编译：`g++ -c factMain.cpp`，产出 `factMain.o` 文件。
2. 只编译：`g++ -c fact.cpp`，产出 `fact.o` 文件。
3. 链接：`g++ factMain.o fact.o`，产出 `a.out` 文件。
...
cout << s1 << endl;
cout << "strlen(s1) = " << strlen(s1) << endl; // 1

// 对于 C 数组，C++ 没有提供方法直接计算其 size，可以借助 sizeof()、begin()、end() 间接计算其长度。
// 编译器会在字符数组结尾添加一个空字符 '\0'，因此数组长度会比它的内容多 1
int length = sizeof (s1) / sizeof (s1[0]);
cout << "length of s1 = " << length << endl; // 2
int length2 = std::end(s1) - std::begin(s1);
cout << "length of s1 = " << length2 << endl; // 2

wchar_t s2[] = L"abc"; // 宽字符数组

int a1 = 10;
unsigned int a2 = 10u;
long a3 = 10L;
unsigned long a4 = 10UL;
int a5 = 012; // 0 开头的整数表示八进制数
int a6 = 0xC; // 0x 开头的整数表示十六进制数

double b1 = 3.14;
float b2 = 3.14f;
long double b3 = 3.14L;
double b4 = 10.;
double b5 = 10e-2;
long double b6 = 3.14e0L;
...
引用即别名，是已存在的对象的另一个名字。定义引用时，程序把引用和它的初始值绑定在一起。无法令引用重新绑定到另一个对象！
...
A reference refers to another type. A pointer points to another type.

引用本身不是一个对象；指针本身是一个对象，允许对指针进行赋值和拷贝。
...
空指针不指向任何对象，在试图使用一个指针之前，应该检查它是否为空。在现行标准下，最好使用 `nullptr`，避免使用 `NULL`。

```cpp
int *p1 = nullptr; // literal
int *p2 = 0; // literal
int *p3 = NULL; // NULL 是预处理变量 (preprocessor variable)，定义在 cstdlib，它的值就是 0；preprocessor 预处理器是运行于编译过程之前的一段程序。
```
...
int i = 42;
const int &r3 = i; // ✅ 允许 const int& 绑定到普通 int 对象上，但不能通过 r3 修改变量 i 的值
i = 84; // 虽然不能通过 r3 修改变量 i 的值，但 i 毕竟不是常量，可以通过其它途径修改
std::cout << r3 << std::endl; // 84

const int &r4 = 42; // ✅ 允许 const int& 绑定到字面值
const int &r5 = r3 * 2; // ✅
```
...
constexpr 修饰的指针是常量指针：`constexpr int *q = nullptr;`
...
头文件通常包含那些只能被定义一次的实体，比如类、`const`、`constexpr`。
...
当预处理器看到 `#include` 标记时，就会用指定的头文件内容代替 `#include`。
...
```cpp
char c = 'c';
bool b = isalpha(c); // 函数位于 cctype 头文件
cout << b << endl;
```
...
模板的作用是，告诉编译器如何生成类或函数。编译器根据模板创建类或函数的过程，称为实例化 (instantiation)。
...
```cpp
const char *pc;
char *p2 = const_cast<char*>(pc); // 可以编译，但通过 p 写值是未定义的行为，非常危险！
```
...
当形参不是引用类型，实参的值被拷贝给形参，称为值传递 (passed by value)。
...
1. 实参们的类型相同：initializer_list (c++ 11)
2. 实参们的类型不同：可变函数模板（ch16.4 介绍）
3. 省略符形参（仅用于访问某些特殊的 C 代码）
...
函数可以返回引用，但不要返回局部对象的引用或指针，因函数完成后它的内存空间也被释放掉。
...
assert 的行为依赖于 NDEBUG 预处理变量，如果定义了这个变量，assert 将什么也不做。命令行提供 NDEBUG 的方法：`g++ -D NDEBUG main.cpp`
...
注意，`(*pt)` 圆括号必不可少，否则的话，声明的就是一个返回类型为 bool 指针的函数。
...
struct Sales_data {
public:
    std::string isbn() const { // 隐式内联函数
        return bookNo; // 隐式 this 指针：this->bookNo
    }
...
vector：`capacity()` 告诉我们容器在不扩容的情况下，可以保存多少元素。`reserve(n)` 分配至少能容纳 n 个元素的内存空间。`shrink_to_fit()` 将 `capacity()` 减少为同 `size()` 的大小。这只是一个请求，标准库并不保证退还内存。
             ...
             char c = ' ')
{
    ...
    // lambba
    // 编译器生成了一个与 lambda 对应的新的（未命名）类，并定义了该类的一个对象
    // 捕获变量 sz，是 lambda 生成的类的数据成员
    // 类似于函数调用时的参数传递，变量的捕获方式可以是值或者引用
    auto f = [sz](const string &a) {
        return a.size() >= sz;
    };
    // 值捕获的前提是变量可以拷贝
    // 被捕获的值是在 lambda 创建时拷贝，而不是调用时拷贝，随后对 sz 进行修改不会影响 lambda 内的值
    sz = 100; // 不会影响 lambda 内的值

    vector<string>::iterator wc = find_if(words.begin(), words.end(), f); // 返回第一个符合 f 的迭代器
    // 当以引用方式捕获变量时，必须保证在 lambda 执行时变量是存在的！
    ...
    // = 表示默认以值捕获变量
    // & 表示默认以引用捕获变量
    auto f2 = [=](const string &a) {
        return a.size() >= sz;
    };
    // 对于值捕获的变量，如果我们想要改变其值，就要加上关键字 mutable
    auto f3 = [sz](const string &a) mutable {
        ++sz;
        return a.size() >= sz;
    };
    // 对于引用捕获的变量，是否能修改取决于引用的是否是 const 类型
}
...
对于无序容器 `unordered_map`, `unordered_set`, `unordered_multimap`, `unordered_multiset`（C++11），不是使用比较运算符来组织元素，而是使用一个哈希函数和 key 的 `==` 运算符。
...
当使用无序容器存储键值对时，会先申请一整块连续的存储空间（标准库通常会选用 vector 容器），存储链表的头指针，各键值对真正的存储位置是各个链表的节点。

![img](/assets/images/9A6641C9-4258-430B-810D-6AEC6A60A3DB.jpg)
...
静态内存用来保存定义在任何函数之外的变量、局部 static 对象、类的 static 数据成员；

栈内存用来保存定义在函数内的非 static 对象。

除了静态内存和栈内存，程序还有堆内存，用来存储动态分配的对象。动态对象的生命期由程序来控制。

## 直接管理内存

C++ 中动态内存的管理是通过 `new` 和 `delete` 这一对运算符来实现的。
...
int main() {
    ...
    delete pi; // pi 必须指向一个动态分配的对象或是空指针，释放非 new 分配的内存、或是相同的指针值被释放多次，其行为都是未定义的！
    delete pi2;
    delete ps;
    delete pv;
...
函数的退出有两种可能，正常结束返回或者发生了异常。一种简单而有效的确保资源被释放的方法是使用智能指针。

```cpp
void f() {
    int *ip = new int(42);
    throw runtime_error("error");
    delete ip; // 发生了异常，且函数 f 以外没有指针指向这块内存，永远也无法释放了，造成内存泄漏
}
```

## 智能指针

为了更容易、更安全地使用动态内存，C++ 11 新标准库提供了两种智能指针：`shared_ptr` 允许多个指针指向同一个对象；`unique_ptr` “独占”所指向的对象。定义在 memory 头文件中。
...
可以认为每个 `shared_ptr` 都有与之关联的“引用计数器”。拷贝一个 shared_ptr 会使引用计数增加；一旦 `shared_ptr` 的引用计数变为 0，它就会自动释放自己管理的对象。
...
copy-assignment constructor, move-assignment constructor 定义了当用同类型的另一个对象赋值给本对象时做什么。`Foo& operator=(const Foo&);`

destructor 定义了类销毁时做什么。`~Foo()`

默认情况下编译器会为我们合成拷贝构造函数。合成拷贝构造函数，会从给定对象中依次将每个非 static 成员拷贝到正在创建的对象中。拷贝初始化发生在：

1. = 赋值时
2. 将对象作为实参，传递给非引用类型的形参
3. 从返回类型为非引用类型的函数返回一个对象
...
基类都应该定义一个 virtual 的析构函数，即使它不执行任何操作。原因是当我们 `delete` 一个动态分配对象的指针时，将执行虚构函数。但由于动态绑定的存在，可能出现指针的静态类型与被删除对象的动态类型不符的情况。例如我们 `delete` 一个 `Qoute*` 类型的指针，但实际它指向的是 `Bulk_qoute` 对象，编译器必须清楚它应该执行 `Bulk_qoute` 的析构函数。和其它函数一样，我们通过在基类中将析构函数定义成虚函数，以确保执行正确的版本：
...
union（联合）是一种特殊的类。一个 union 可以有多个数据成员，但是在任意时刻只有一个数据成员可以有值。当给某个成员赋值之后，其它数据成员就变成未定义了。

分配给一个 union 的存储空间至少要能容纳它的最大的数据成员。
