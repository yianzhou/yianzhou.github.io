...
检索增强生成（RAG, Retrieval Augmented Generation）是指对大型语言模型输出进行优化，使其能够在生成响应之前引用训练数据来源之外的权威知识库。这个过程主要分为两个阶段：准备阶段（存）和查询阶段（取）。

一、准备数据。切片（Chunking），把 100 页的员工手册切分成几百个小的文本块（Chunk）。然后向量化（Embedding），利用 AI 模型（Embedding Model），把这些文字变成计算机能读懂的“数字坐标”（向量）。例如块 A 变成了 [0.12, -0.45, 0.88, ...] 这样一串数字，存入向量数据库中。

二、查询阶段。

1. 用户提问向量化：用户输入“我生病了想请假，需要提供医院证明吗？”系统调用**同一个** Embedding 模型，把这句大白话也变成一串数字向量。
2. 向量相似度搜索：去向量数据库里和与员工手册的文本块进行比对，找含义（语义）距离最近的片段。
3. 召回：系统锁定了块 B（及其前后文），并将其从数据库中“捞”了出来。
4. 构建提示词（Prompt Engineering）：系统把你原来的问题，和捞出来的“块 B”拼装成给大模型的 Prompt。
5. 大模型生成回答。
