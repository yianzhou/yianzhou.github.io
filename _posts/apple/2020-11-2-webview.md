...
![img](/assets/images/D2CA9C16-BD9E-447B-BB61-C3B1F12B25F0.jpg)
...
一、多路复用的单一长连接。在 HTTP/2 中，客户端向某个域名的服务器请求的过程中，只会创建一条 TCP 连接，即使这页面可能包含上百个资源，避免了创建多个 TCP 连接带来的网络开销。HTTP2 把要传输的信息逻辑上分成了一个个数据流，每个数据流都以消息的形式发送，而消息又由一个或多个帧组成。多个帧之间可以乱序发送，根据帧首部的流标识可以重新组装。

二、头部压缩：对消息 Header 采用 HPACK 进行压缩传输。
