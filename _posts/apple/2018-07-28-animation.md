...
class MyViewController: UIViewController {
    ...
    @objc func nestedTransaction(_ sender: UIButton) {
        ...
        CATransaction.begin() // push another transaction to the stack, transaction can be nested
        ...
        /*
         在 UIView 中封装了这个方法，+beginAnimations:context: 和 +commitAnimations
         在 iOS 4+，苹果对 UIView 添加了一种基于 block 的动画方法：+animateWithDuration:animations:completion:
         block 中所有属性的改变都会自动被 CATransaction 包含，可以避免开发者手动对 +begin 和 +commit 匹配容易造成的失误。
         */
    }
}
```
