...
## 隐式动画
Core Animation 基于一个假设，即屏幕上的任何东西都可以（或者可能）做动画。\
...
隐式动画是针对属性的更改，它很基础、也很简单，不过它并不能涵盖所有的动画类型。而**显式动画**能够对一些属性做指定的自定义动画、指定动画运行的路径等等。
...
iOS 10+，使用 `UIViewPropertyAnimator`，暴露出了很多以前要用显式动画才能做到的特性，比如时间曲线等。
