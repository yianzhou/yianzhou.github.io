---
title: "开发及调试"
categories: [Apple]
---

<!-- prettier-ignore -->
* Do not remove this line (it will not be displayed)
{:toc}

# 在 Xcode 中打开 Terminal

1\. Create executable shell script with the following contents and save it anywhere

```bash
#!/bin/bash
open -a Terminal "`pwd`"
```

2\. Add execute permissions to your script: `chmod +x <YourShellScript>`

3\. In the Xcode menu bar, Go to Xcode -> Preferences -> Behaviors.

4\. Add a Custom behavior, name it "Open Terminal". (Optional) Configure a hotkey for the behavior.

5\. Checkmark Run, select you script from step 1.

# 控制台

打开 Console.app，在左边侧边栏“设备”里找到正在调试的设备，可查看 NSLog

# 打印

将字典打印成 json：

```shell
p print(String(data: try! JSONSerialization.data(withJSONObject: dict, options: .prettyPrinted), encoding: .utf8 )!)
```

```objc
NSString *str = @"Susan";
NSLog(@"指针所指对象的内存地址：%p", str); // 堆区，低地址区
NSLog(@"指针本身的内存地址：%p", &str); // 栈区，高地址区
```

# 快捷键

`Command+Shift+J` 定位到当前文件

`Command+Option+/` 模板注释

查看所有断点：View → Navigators → Show Breakpoint Navigator. `Command + 8`

Esc 键手动弹出自动补全

# lldb 命令

创建变量：`e var $map = ["a": 1]`

打印变量：`po $map`

`$po bt all` 打印全部堆栈信息（print object, backtrace）

`(lldb) print <expr>` : 打印需要查看的变量，简写 p，你会发现输出的信息中带有$1、$2 的字样。实际上，lldb 的每次查询结果会保存在一些持续变量中，(\$[0-9]+)，之后可以直接使用、修改这些值。

# 崩溃符号化

一、导出崩溃日志，设置 -> 隐私 -> 分析与改进 -> 分析数据，根据崩溃的发生时间和 App 名字，找到 ips 文件，点击查看，然后通过导航栏的分享按钮发送出来。

二、Xcode 中必须要有对应版本系统的 iOS DeviceSupport 文件，否则符号化会失败。打开 DeviceSupport 文件夹：

`open /Applications/Xcode.app/Contents/Developer/Platforms/iPhoneOS.platform/DeviceSupport`

下载 DeviceSupport：<https://github.com/iGhibli/iOS-DeviceSupport>

三、找到发生崩溃的测试包对应的 dSYM 文件。将崩溃日志和 dSYM 文件放到同一个目录下，然后采用 symbolicatecrash 命令进行符号化。symbolicatecrash 是 Xcode 自带的符号化工具，但是不在系统的 PATH 内，因此运行该命令需要修改 PATH 变量，在终端执行如下命令：

```shell
export DEVELOPER_DIR="/Applications/Xcode.app/Contents/Developer"
export PATH=$PATH:/Applications/Xcode.app/Contents/SharedFrameworks/DVTFoundation.framework/Versions/A/Resources/
```

执行命令：`symbolicatecrash xxx.crash -o yyy.crash`

# 在 App 内读取编译信息

1\. 增加 produceID.sh、removeID.sh、buildID.txt 文件

2\. Build Phases 增加编译前 script 脚本，执行 produceID.sh，生成唯一 id 保存至 buildID.txt

3\. Build Phases 增加编译后 script 脚本，执行 removeID.sh，将 buildID.txt 清空，避免每次编译导致 git 提示文件被修改

4\. App 内读取 buildID.txt 内容

# Info.plist

`UIFileSharingEnabled` 在 iTunes File Sharing 中可见

`NSAppTransportSecurity` 允许非 HTTPS

# 网络

网络调试：<http://requestbin.net/>，<https://httpbin.org/get>，<https://httpbin.org/post>

用 POSTMAN 调试时，如果解析不到 JSON，看返回的结果是不是 HTML 的页面形式。

模拟器用的网络调节器：先[下载](https://developer.apple.com/download/more/?=for%20Xcode)，copying the .prefPane file to: `/Library/PreferencePanes`

[Charles 在线破解工具](https://www.zzzmode.com/mytools/charles/)
