...
分页就是把地址空间切分成固定大小的单元，这样我们就不用去考虑堆和栈具体申请多少空间，只要考虑需要多少页就可以了。由 CPU 生成的地址通常称为逻辑地址，而由内存单元看到的地址（即加载到内存的地址寄存器中的地址）通常称为物理地址，每个进程会有一个分页表记录这个对应关系，在进程调度过程中，上下文切换阶段会做页表的切换。虚拟地址页（page）到物理地址帧（Frame）的转换是通过硬件 MMU 实现的，
...
通过 Jetsam 文件来查看占用内存最大的进程、每个进程占用的内存、被杀死的进程以及它被杀死的原因。
...
- (void)didReceiveMemoryWarning {
    ...
    if( kr == KERN_SUCCESS ) {
        ...
        NSLog(@"Error with task_info(): %s", mach_error_string(kr));
    }
}
```
