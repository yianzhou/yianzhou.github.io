...
苹果注册了一个 Observer 监测 BeforeWaiting（即将进入休眠）事件，这个Observer的回调函数是 _UIGestureRecognizerUpdateObserver()，其内部会获取所有刚被标记为待处理的 GestureRecognizer，并执行 GestureRecognizer 的回调。
...
typedef CF_OPTIONS(CFOptionFlags, CFRunLoopActivity) {
    ...
    kCFRunLoopExit = (1UL << 7),// RunLoop 已经推出
