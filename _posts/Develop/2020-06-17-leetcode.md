...
在计算机科学中，树（英语：tree）是一种抽象数据类型（ADT），它具有以下的特点：
...
最大路径和 = 左子树最大路径和 + 右子树最大路径和 + 根节点，这是一个后序遍历
...
99-恢复二叉搜索树：在一次遍历中找到被交换的数，然后将它们交换即可。
...
98-验证二叉搜索树：在中序遍历的时候，实时检查当前节点的值是否大于前一个中序遍历到的节点的值即可。
...
108-将有序数组转换为二叉搜索树：有序数组转换成 BST 有多种答案。
...
230-二叉搜索树中第 K 小的元素：执行中序遍历得到有序数组，取 res[k-1] 即可。
