...
知识准备一：最大公约数。gcd 函数满足交换律：`gcd(a, b) = gcd(b, a)`；约定 `gcd(a, 0) = a`。
...
快速排序算法在平均状况下有着不错的表现，但是对于基准值的选择十分敏感，最坏情况下的算法复杂度是 O(n^2)。C++ std sort 的实现选择了首部、尾部和中部三个元素的中值作为 pivot。
...
本题要掌握的实际是快速选择算法，它与快速排序一样，都是由计算机科学家托尼·霍尔发明的。
