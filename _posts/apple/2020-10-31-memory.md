...
![img-40](/assets/images/D4A8A432-60B4-41DF-8B6B-5E7364B47850.png)

进程只能访问自己的地址空间，分页就是把地址空间切分成固定大小的单元。

由 CPU 生成的地址称为逻辑地址，加载到内存的地址寄存器中的地址称为物理地址。

虚拟地址页（Page）到物理地址帧（Frame）的转换是通过硬件 MMU 实现的。每个进程会有一个分页表记录这个对应关系，在进程调度过程中，上下文切换阶段会做页表的切换。

分页交换 Swapping pages: A page out operation moves a page from memory to the backing store; the reverse process is known as a page in.
...
Clean memory is data that can be paged out. **Memory-mapped files** 内存映射文件 are files on disk and loaded in the memory. If you use read-only files, these are always going to be clean pages. These could be images, data blobs, training models, and frameworks (DATA CONST section).
